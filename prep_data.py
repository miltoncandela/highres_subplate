...
def main(input_data_path, output_data_path, mask_file, GA_info = None, input_size = 192, split_method = 'train_valid_split', n_fold = 10, fold_path = '0.txt'):
    ...
    df = pd.DataFrame({'MR': [x[30:] for x in MR_list], 'GT': [x[30:] for x in GT_list]})
    ...
    if split_method == 'manual_strat':
        ...
        GT_list_valid = [x for x in GT_list if x.split('_')[-1] in [y + '.nii' for y in subject_list]]
        MR_list_valid = [x for x in MR_list if x.split('_')[-1] in [y + '.nii' for y in subject_list]]

        GT_list_train = list(set(GT_list).difference(GT_list_valid))
        MR_list_train = list(set(MR_list).difference(MR_list_valid))
        
        print('Training:', MR_list_train)
        print('Testing:', MR_list_valid)
    ...
    elif split_method == 'train_valid_split':
        print(split_method)
        ...
        MR_list_train, MR_list_valid, GT_list_train, GT_list_valid  = list(train_test_split(MR_list, GT_list, test_size=0.1, random_state=7))
...
def preprocess_volume(MR_volume, GT_volume, input_size, view, mask_img, mask_min, mask_max, repre_size, flip):
    ...
    if view == 'axi':
        ...
        Y_label = np.zeros([repre_size[2], input_size, input_size,7])
    elif view == 'cor':
        X_image = np.zeros([repre_size[1], input_size, input_size,1])
        Y_label = np.zeros([repre_size[1], input_size, input_size,7])
    elif view == 'sag':
        X_image = np.zeros([repre_size[0], input_size, input_size,1])
        Y_label = np.zeros([repre_size[0], input_size, input_size,4])
    ...
    if (view == 'axi') | (view == 'cor'):
        ...
        left_sp_loc = np.where((img2>4.5)&(img2<5.5))
        right_sp_loc = np.where((img2>3.5)&(img2<4.5))
        img3[back_loc]=1
        ...
        sp_loc = np.where((img2>4.5)&(img2<5.5)|(img2>3.5)&(img2<4.5))
        img3[back_loc]=1
...
if __name__ == "__main__":
    ...
    parser.add_argument('-Input_dir', '--input',action='store',dest='input_dir',type=str, required=True, help='Input path for raw MRI files(*.nii)')
    parser.add_argument('-Output_dir', '--output',action='store',dest='output_dir',type=str, required=True, help='Output path for organized dataset(*.npy)')
    ...
    parser.add_argument('-split', '--split', dest='split_method', type=str, default='manual_strat', help='training/validation data splitting method')
