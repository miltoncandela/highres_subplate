...
sub_hs = [file.split('_')[6] for file in os.listdir(highres_segs) if len(file.split('_')) > 6]

print('  Nuc Seg')
print('HR', len(sub_hn), len(sub_hs))
print('')

df_cp = pd.DataFrame(sub_hn, index=range(len(sub_hn)), columns=['ID'])
df_cp['MRN'] = [pross_sub(sub) for sub in df_cp.ID]
hs_MRN = [float(sub) for sub in df_cp.MRN if sub is not None]
...
'''
print("--- HyukJin's data ---")
df_cp = pd.read_csv('/neuro/labs/grantlab/research/HyukJin_MRI/2024_Data/CP_seg/48_subj/list_48_subj.csv')
hs_MRN = [pross_sub(sub.split('/')[0]) for sub in df_cp.ID]
df_cp['MRN'] = hs_MRN
hs_MRN = [float(sub) for sub in hs_MRN if sub is not None]
df_cp = df_cp.dropna(axis=0)
