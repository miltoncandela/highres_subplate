...
class Unet_network:
    def __init__(self, input_shape, out_ch, loss='dice_loss', metrics=['dice_coef', 'dis_dice_coef'], style='basic', ite=2, depth=4, dim=32,
                  weights='', init='he_normal',acti='elu',lr=1e-4, dropout=0, transfer_learn = False, mname= ''):
        ...
        self.kernel2 = None
        self.mname = mname
        self.transfer_learning = transfer_learn
    ...
    def UNet(self):
        ...
        if self.transfer_learning:
            ...
            # Encoder pre-trained, train decoder on 0-initialized weights
            if self.mname == 'B' or self.mname =='D':
                encoder_layer = True
                for layer in model.layers:
                    if 'conv2d_14' in layer.name: # Starting decoder layer
                        encoder_layer = False
                    
                    # If a encoder layer, freeze their weights
                    if encoder_layer:
                        layer.trainable = False
                    
                    # If a decoder layer, set their weights to 0
                    if not encoder_layer:
                        layer.set_weights([tf.zeros_like(w) for w in layer.get_weights()])

            # Encoder & decoder pre-trained, train decoder on pre-trained
            elif self.mname == 'C' or self.mname == 'E':
                encoder_layer = True
                for layer in model.layers:
                    if 'conv2d_14' in layer.name: # Starting decoder layer
                        encoder_layer = False
            ...
            # Encoder & decoder pre-trained, train last predictive layer
            elif self.mname == 'F':
                for layer in model.layers:
                    layer.trainable = False
            
            # Model G is just pre-trained without changing weights or freezing layers
            elif self.mname == 'G':
                pass
                
            out_ch_sp = 7 if self.out_ch == 5 else 4
            last_conv = model.layers[-2].output
            o = Conv2D(out_ch_sp, 1, activation='softmax')(last_conv)
...
def make_dic(img_list, gold_list, mask, dim,flip=0):
    ...
    if dim == 'axi':
        ...
        seg = np.zeros([repre_size[2]*len(img_list), max_repre, max_repre,7])
    elif dim == 'cor':
        dic = np.zeros([repre_size[1]*len(img_list), max_repre, max_repre,1])
        seg = np.zeros([repre_size[1]*len(img_list), max_repre, max_repre,7])
    elif dim == 'sag':
        dic = np.zeros([repre_size[0]*len(img_list), max_repre, max_repre,1])
        seg = np.zeros([repre_size[0]*len(img_list), max_repre, max_repre,4])
    else:
...
def make_sum(axi_filter, cor_filter, sag_filter, input_name, result_loc):
    ...
    left_sp = np.zeros(np.shape(axi.get_fdata()))
    right_sp = np.zeros(np.shape(axi.get_fdata()))
    ...
    result = np.concatenate((bak[np.newaxis,:], left_in[np.newaxis,:], right_in[np.newaxis,:],
                            left_plate[np.newaxis,:], right_plate[np.newaxis,:], left_sp[np.newaxis,:], right_sp[np.newaxis,:]),axis=0)
